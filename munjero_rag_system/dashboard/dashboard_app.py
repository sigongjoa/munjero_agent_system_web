...
@app.route('/api/send_to_chatgpt_textfield', methods=['POST'])
def send_to_chatgpt_textfield():
    print("Dashboard: /api/send_to_chatgpt_textfield POST request received", flush=True, file=sys.stderr)
    data = request.get_json()
    text_content = data.get('text_content')

    if not text_content:
        print("Dashboard: Text content is missing", flush=True, file=sys.stderr)
        return jsonify({"error": "Text content is required"}), 400

    command = {
        "type": "SEND_TO_CHATGPT_TEXTFIELD",
        "text_content": text_content
    }
    print(f"Dashboard: Pushing command to AGENT_COMMANDS_LIST: {command}", flush=True, file=sys.stderr)
    redis_client.lpush(AGENT_COMMANDS_LIST, json.dumps(command))
    print("Dashboard: Command pushed to Redis", flush=True, file=sys.stderr)
    return jsonify({"message": "Text content sent to Chrome extension for textfield input."}), 200
