...
import { subjectSocialData } from '../services/subjectSocialData';

const SubjectSocial: React.FC = () => {
    ...
    return (
        <div className="animate-fade-in bg-white p-6 sm:p-10 rounded-2xl shadow-lg border border-slate-200 w-full">
            <h1 className="text-3xl font-bold text-slate-900 mb-4 border-b pb-2">고교 교과과목: 사회</h1>
            <p className="mb-6 text-slate-600">2022 개정 교육과정에 따른 사회과 교육과정의 주요 내용입니다.</p>
            <div>
                {renderContent(subjectSocialData)}
            </div>
        </div>
    );
};

export default SubjectSocial;