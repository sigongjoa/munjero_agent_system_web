...
export default function ChatGPTAutomation() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!prompt || !taskId) {
      setError('Prompt and Task ID are required.');
      return;
    }

    try {
      const res = await fetch('/api/trigger-chatgpt-image', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="p-4">
      <h2 className="text-2xl font-bold mb-4">ChatGPT Image Generation Automation</h2>
